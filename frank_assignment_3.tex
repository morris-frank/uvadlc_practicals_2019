...
As we have an VAE not an standard autoencoder we actually model the distribution \(q_\φ(Z|X)\) to have a generalizing model in which we can interpolate inside the embedding space of \(z\).
Having the continuos z-space is the models regularization.
Our metric for how good our projection from \(x\) to \(z\) is, is the KL divergence from the actual \(p(Z|X)\).
Therefore it is fair to say that the divergence is called the regularization loss.
